...
$(document).ready(function () {
    ...
    if (COOKIE_OBJ.get('GEO_LATITUDE') != "" && COOKIE_OBJ.get('GEO_LONGITUDE') != "") {
        latlongLoad = {
            lat: parseFloat(COOKIE_OBJ.get('GEO_LATITUDE')),
            lng: parseFloat(COOKIE_OBJ.get('GEO_LONGITUDE'))
        };
...
function hideSpinner(fieldId) {
   // $(".progress-indeterminate").remove();
    $("#" + fieldId).next('.progress-indeterminate').remove();
    showClearButton();
}

function hideSpinnerOnly(fieldId) {
    //$(".progress-indeterminate").remove();
    $("#" + fieldId).next('.progress-indeterminate').remove();
}
...
function createLoader_bkp(newfieldId) {
    ...
    if(newfieldId != 'undefined') {
        ...
        if ($("#" + newfieldId).next(".progress-indeterminate").length == 0) {
...
function createLoader(newfieldId) {
    if (newfieldId !== 'undefined' && newfieldId !== undefined) {
        var element = $("#" + newfieldId);
        if (element.length === 0) {
            console.error("Element with ID " + newfieldId + " not found.");
            return;
        }
        
        var mywidth = element.outerWidth();
        var mleft = element.css('margin-left').replace('px', '');
        var mbottom = element.css('margin-bottom');
        var rightonly = element.css('right');
        var bgcolor = element.css('background-color');
        
        if (element.next(".progress-indeterminate").length === 0) {
            element.after(
                '<div class="progress progress-indeterminate"><div class="progress-bar progress-bar-striped indeterminate"></div></div>'
            );
            
            var totalWidth = newfieldId === "vServiceAddress" ? '55.50%' : '100%';
                    ...
                    var listofautocomplete = $("#" + fieldId).autocomplete({
                        ...
                        select: function (event, ui) {
                            ...
                            if(ui.item.Permission == 0){
                                fetchLocation();
                                set_lat_long_values(ui.item.value , fieldId);
                                hideSpinnerOnly(fieldId);
                                return false;
...
function set_lat_long_values(data_response , fieldId) {
    createLoader(fieldId);
    if (data_response.PlaceId || data_response.place_id) {
        ...
        getDatafromApi(buildPlaceDetail(PlaceID, data_response.vServiceId ? data_response.vServiceId : ''), function (data_responses, vReturnData) {
            ...
            place_lat_log_value(address, latitude, lngitude,fieldId);
            if (newCallback != '' && newCallback != 'undefined' && newCallback != undefined) {
                newCallback(latitude, lngitude, address, address_components);
            }
        });
    } else {
        place_lat_log_value(data_response.address, data_response.latitude, data_response.longitude , fieldId);
        if (newCallback != '' && newCallback != 'undefined' && newCallback != undefined) {
            newCallback(data_response.latitude, data_response.longitude, data_response.address, data_response.address_components);
        }
    }
}

function place_lat_log_value(address, latitude, longitude,fieldId) {
    ...
    var addFieldary = new Array();
    for(s=1;s<=50;s++){
        addFieldary.push('tAddStop'+s);
    }
   

    if (newfieldId == "from" || newfieldId == "to" || newfieldId == "from2" || newfieldId == "to2" || (addFieldary.includes(newfieldId) == true) ) {
        $("#" + newfieldId + "_lat_long").val('(' + latitude + ', ' + longitude + ')').trigger('change');
        $("#" + newfieldId + "_lat").val(latitude).trigger('change');
        $("#" + newfieldId + "_long").val(longitude).trigger('change');
        $("#" + newfieldId + "_lat_long_frm").val('(' + latitude + ', ' + longitude + ')').trigger('change');
        $("#" + newfieldId + "_lat_frm").val(latitude).trigger('change');
        $("#" + newfieldId + "_long_frm").val(longitude).trigger('change');
    } else {
